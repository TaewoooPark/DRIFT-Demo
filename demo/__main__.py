...
from .server import Bus, Handler, ThreadingHTTPServer, udp_listener
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--events-port", type=int, default=9801, help="UDP port for worker events")
    ...
    threading.Thread(target=udp_listener, args=(bus, args.events_port), daemon=True).start()
    ...
    for i, p in enumerate(node_ports):
        env = {**os.environ,
               "DRIFT_DEMO_EVENTS": f"127.0.0.1:{args.events_port}",
    ...
    endpoints = [{"name": f"n{i}", "host": "127.0.0.1", "port": p}
                 for i, p in enumerate(node_ports)]
    head = DemoHead(args.model, args.dtype, endpoints, max_new_tokens=args.max_new_tokens)
    ...
    def build():
        try:
            head.build()
            print(f"[demo] ready — {args.model} split across {args.nodes} worker(s)", flush=True)
